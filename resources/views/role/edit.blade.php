				...
				<div class="card-body">
					<form action="{{ Request::url() }}" method="post" data-parsley-validate>
						...
						<div class="form-group row">
							...
							<div class="col-md-4">
								<input type="text" class="form-control" maxlength="45" name="role_name" value="{{ $role->role_name }}" required>
							</div>
						</div>
						<div class="form-group row">
							<label class="col-md-2 col-form-label">Nama Role</label>
							<div class="col-md-4">
								<select class="form-control" name="state" required>
									<option value="1" {{ $role->state == '1' ? 'selected' : '' }}>Aktif</option>
									<option value="0" {{ $role->state == '0' ? 'selected' : '' }}>Non Aktif</option>
								</select>
							</div>
						</div>
						<div class="form-group row">
												...
												<div class="col-md-4">
													...
													@else
														<div class="icheck-primary d-inline">
															<input type="checkbox" id="menu{{ $item->menu_id }}" value="{{ $item->menu_id }}" {{ $role->menu->firstWhere('menu_id', $item->menu_id) ? 'checked' : '' }} name="menu[]">
															<label class="font-weight-normal" for="menu{{ $item->menu_id }}">
																<i class="{{ $item->icon }}"></i> {{ $item->menu_name }}
															</label>
														</div>
												...
												<div class="col-md-8">
													<div class="ml-5">
														@foreach ($item->permission as $permission)
															<div class="icheck-primary d-inline">
																<input type="checkbox" id="permission{{ $permission->permission_id }}" value="{{ $permission->permission_id }}" {{ $role->permission->firstWhere('permission_id', $permission->permission_id) ? 'checked' : '' }} name="permission[]">
												...
												<div class="row">
													<div class="col-md-4">
														<div class="icheck-primary d-inline ml-4">
															<input type="checkbox" id="menu{{ $submenu->menu_id }}" value="{{ $submenu->menu_id }}" {{ $role->menu->firstWhere('menu_id', $submenu->menu_id) ? 'checked' : '' }} name="menu[]">
