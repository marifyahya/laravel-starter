				...
				<div class="card-body">
					<form action="{{ Request::url() }}" method="post" data-parsley-validate>
							...
							<div class="col-md-4">
								<select name="menu" class="form-control select2" placeholder="Pilih Menu" required>
									...
									@foreach ($menu as $item)
										<option value="{{ $item->menu_id }}" {{ $permission->menu_id == $item->menu_id ? 'selected' : '' }} >{{ $item->menu_name }}</option>
									@endforeach
								</select>
							</div>
						</div>
						<div class="form-group row">
							...
							<div class="col-md-4">
								<input type="text" class="form-control" name="permission_name" placeholder="Nama Permission" value="{{ $permission->permission_name }}" data-parsley-maxlength="50" required>
							</div>
						</div>
						<div class="row">
